...
void setLightPosition(GLuint program, glm::vec3 lightPos)
{
...
void setViewPosition(GLuint program, glm::vec3 eyePos)
{
    glUseProgram(program);
    glUniform3fv(glGetUniformLocation(program, "viewPos"),
        1, glm::value_ptr(eyePos));
}
...
int main()
{
    ...
    GLuint blinnShader = initShader("shaders/blinn.vert", "shaders/blinn.frag");
    GLuint phongShader = initShader("shaders/blinn.vert", "shaders/phong.frag");
    GLuint texblinnShader = initShader("shaders/texblinn.vert", "shaders/texblinn.frag");
    GLuint depthShader = initShader("shaders/simpledepth.vert", "shaders/simpledepth.frag");
    GLuint shadowShader = initShader("shaders/shadowmap.vert", "shaders/shadowmap.frag");
    //GLuint normalBlinnshader = initShader("normalblinn.vert", "normalblinn.frag");

    glUseProgram(texblinnShader);
    setLightPosition(texblinnShader, lightPos);
    setViewPosition(texblinnShader, viewPos);

    glUseProgram(phongShader);
    setLightPosition(phongShader, lightPos);
    setViewPosition(phongShader, viewPos);

    glUseProgram(blinnShader);
    setLightPosition(blinnShader, lightPos);
    setViewPosition(blinnShader, viewPos);
    ...
    //glUseProgram(normalBlinnshader);
    //setLightPosition(normalBlinnshader, lightPos);
    //setViewPosition(normalBlinnshader, viewPos);

    // LabA09 Shadow Map
    glUseProgram(depthShader);
    setLightPosition(depthShader, lightPos);
    setViewPosition(depthShader, viewPos);

    glUseProgram(shadowShader);
    setLightPosition(shadowShader, lightPos);
    setViewPosition(shadowShader, viewPos);
